...
CdCommonClosePrivate (
    ...
    __in PVCB Vcb,
    __in PFCB Fcb,
    __in ULONG UserReference,
    __in BOOLEAN FromFsd
    );
...

{
    ...
    while ((IrpContext = CdRemoveClose( Vcb )) != NULL) {
        ...
        if (PotentialVcbTeardown ||
            ...
            __analysis_assume( CurrentVcb != NULL );
...
#pragma prefast( suppress: 28107, "prefast cannot work out that the VCB is already acquired")
            CdReleaseVcb( IrpContext, CurrentVcb );
